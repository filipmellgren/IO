...
lambda1 <- -1.62
lambda2 <- 3.03
df <- df %>% mutate(Si = pop + pos_gpop * lambda1 + neg_gpop * lambda2)
...
df <- df %>% mutate(benchmark = Si - iSN)
df <- df %>% mutate(ambition = iSNplus1 - Si)
