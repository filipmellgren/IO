...
$S_N = \frac{\hat{\gamma}_1 + \hat{\gamma}_L \bar{W}_L + \sum^N_{n=2}\hat{\gamma_n}} {\hat{\alpha}_1 + \bar{X}\hat{\beta} - \sum_{n=2}^N \hat{\alpha}_n}$
...
Motivate why we don't include non significant variables in here. (dont want to include noise)


```{r}
# beta1*s_kids + beta2*s_young + beta3*s_pens+ beta4*s_women + beta5*cons_pc + beta6*wage
Xbar <- df %>% dplyr::select(s_kids, s_young, s_pens, s_women, cons_pc, wage) %>% summarize(mean())
Wbar <- 
