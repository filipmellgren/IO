...
sigma_fe <- lm.nest[["coefficients"]][["ln_mshare_s"]]
...
df %>% group_by(firm) %>% summarise("Baseline" = abs(mean(e_jj_nl)), "Model F.E." = abs(mean(e_jj_nl_fe))) %>% 
  gather(key = "Specification", value = "value", "Baseline", "Model F.E.") %>% 
  ggplot(aes(x = as.factor(reorder(firm, -value)), y = value, fill = Specification)) +
