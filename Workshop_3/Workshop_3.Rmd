...
```{r merger_function}
post_merger_df <- function(dataframe, mc){
...
dataframe <- dataframe %>% mutate(pderiv_new = alpha_2 * mshare1*((1/(1-sigma))-(sigma/(1-sigma))*mshare_s1-mshare1),
                     markup_new = -(mshare1 / pderiv_new)*mc, # is this where the marginal cost is changing?
...
price_change_table <- rbind(ger_table, gm_table, vw_table) %>% select(Variable, Mean, name) %>% spread(key = Variable, value = Mean) %>% as_tibble() %>% select(-dprice) #%>% 
 # gather("price", "value", -name)
```
```{r}
price_change_table %>% gather("price", "value", -name) %>% mutate(time = if_else(price == "price", 0,1)) %>% ggplot(aes(x = time, y = value)) +
  geom_point() +
  geom_segment(aes(xend = 2, yend = price_new, color = name))
```
...
```{r}
price_change_table %>% ggplot(aes(x = 1, y = price)) +
  geom_point() +
  geom_segment(aes(xend = 2, yend = price_new, color = name))
```
