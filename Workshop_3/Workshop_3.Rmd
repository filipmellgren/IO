...
df <- df %>% mutate(year1 = year, country1 = country) %>% unite(yearcountry, c("year1", "country1"))
```
...
summary_stats <- function(x){
  summary <- rbind(mean(x), sd(x, na.rm = TRUE), min(x, na.rm = TRUE), max(x, na.rm = TRUE)) 
  return(summary)
}
```
...
df %>% 
  select(year, country, co, segment, domestic, firm, qu, price, horsepower, fuel, width, height, pop) %>% map(function(x) summary_stats(x)) %>% as_tibble() %>% t() %>% round(2) %>% as_tibble(rownames = "Variable") %>% rename ("Mean" = V1, "S.e." = V2, "Min." = V3, "Max." = V4 )
```
```{r}
# TODO: Add numbers to each cell
cormatdf <- df %>% select(year, country, domestic, qu, price, horsepower, fuel, width, height, pop)
...
melted_cormat %>% ggplot(aes(x=Var1, y=Var2, fill=value)) + 
  ...
  scale_fill_gradient2(low = "#336666", high = "#664033", mid = "white", 
   midpoint = 0, limit = c(-1,1), space = "Lab", 
    name="Pearson\nCorrelation")
```
Excluded co (model code), segment (ordinal variable), firm
...
#df %>% 
#  select(msize, mshare) %>% map(function(x) summary_stats(x)) %>% as_tibble() %>% t() %>% round(5) %>% #as_tibble(rownames = "Variable") %>% rename ("Mean" = V1, "S.e." = V2, "Min." = V3, "Max." = V4 )
```
...
```
```{r}
alpha_1 <- logit_fit[["coefficients"]][["price"]]
df <- df %>% mutate(own_price_elasticity = alpha_1 * (1 - mshare)*price)

# TODO: fix output
summary_stats(df$own_price_elasticity)
```
```{r}
# TODO: Make it pretty!
df <- df %>% group_by(firm) %>% mutate(avg_e_jj_l = mean(own_price_elasticity))
...
# TODO: fix output
summary_stats(df$e_jj_nl)
```
```{r}
# TODO: add to previous chart
df <- df %>% group_by(firm) %>% mutate(avg_e_jj_nl = mean(e_jj_nl))
...
# summarise
df %>% select(markup, lerner, mc, price) %>% summary_stats()
...
```{r}
df_merger <- df
df_merger <- df_merger %>% mutate(firmnew = if_else(firm == 15 & country == 3 & year >= 1998, 26, firm),
...
df_merger <- df_merger %>% group_by(segment_new, yearcountry) %>% mutate(tot_s1 = sum(qu_new),
                                                            mshare_s1 = qu_new / tot_s1)

```
```{r}
df_merger %>% filter(year == 1998, country == 3) %>%
  select(mshare_s, mshare_s1) %>% map(function(x) summary_stats(x)) %>% as_tibble() %>% t() %>% as_tibble(rownames = "Variable") %>% rename ("Mean" = V1, "S.e." = V2, "Min." = V3, "Max." = V4 )
```
## Post merger
```{r}
# TODO: double check we should be using alpha_2 and not something else
df_merger <- df_merger %>% mutate(pderiv_new = alpha_2 * mshare1*((1/(1-sigma))-(sigma/(1-sigma))*mshare_s1-mshare1),
...
```
```{r}
df_merger %>% filter(year == 1998, country == 3) %>% drop_na() %>% 
  select(price, price_new, dprice) %>% map(function(x) summary_stats(x)) %>% as_tibble() %>% t() %>% as_tibble(rownames = "Variable") %>% rename ("Mean" = V1, "S.e." = V2, "Min." = V3, "Max." = V4 )

# GM
df_merger %>% filter(year == 1998, country == 3, firm == 15) %>% drop_na() %>% 
  select(price, price_new, dprice) %>% map(function(x) summary_stats(x)) %>% as_tibble() %>% t() %>% as_tibble(rownames = "Variable") %>% rename ("Mean" = V1, "S.e." = V2, "Min." = V3, "Max." = V4 )

# VW
df_merger %>% filter(year == 1998, country == 3, firm == 26) %>% drop_na() %>% 
  select(price, price_new, dprice) %>% map(function(x) summary_stats(x)) %>% as_tibble() %>% t() %>% as_tibble(rownames = "Variable") %>% rename ("Mean" = V1, "S.e." = V2, "Min." = V3, "Max." = V4 )
```
...
Create a function that takes a data frame and a marginal cost that then returns an updated data frame
