...
```{r}
pre_post_table <- pre_post_table %>% select(-treatment) %>%  spread(price_type, price) 
```
```{r}
pre_post_table %>%
  ggplot(aes(x = as.factor(0), y = price)) +
  geom_point() +
  geom_point(aes(x = as.factor(1),y = price_new)) +
  geom_point(aes(x = as.factor(1), y = efficient_price)) +
  geom_segment(aes(xend = as.factor(1), yend = price_new),color = "#e3120b") +
  geom_segment(aes(xend = as.factor(1), yend = efficient_price), color = "#244747") +
  theme_economist() + scale_fill_economist() + scale_color_economist() +
