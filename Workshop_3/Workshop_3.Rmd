...
df <- df %>% mutate(yearcountry = paste(year, country),
                    i = 1)
df <- df %>% group_by(yearcountry) %>% mutate(agg.cty.yr.qu = sum(qu),
                                              N = sum(i)) %>% ungroup()
...
for (c in 1:5) {
 descriptive_table <-  rbind(descriptive_table, df %>% filter(country == c) %>% 
  select(country, year, domestic, qu, price, princ, horsepower, fuel, pop, wdomestic, wfuel, whorsepower, wweight, wwidth, wheight, wpr, wprice, wprinc, wsegment) %>% 
    mutate(pop = pop/1000000) %>%
...
```{r}
# Adds price to the X
X <- glue("price", "year", "horsepower", "fuel", "width","height", "domestic", "year",
          "country1", "country2", "country3", "country4", .sep = "+")
...
formula <- glue("ln_mshare ~", X)
lm.nest <- lm(formula, data = df)
fe_formula <- glue("ln_mshare ~", X_fe)
...
Figure_data <- df %>% rename("Baseline" = e_jj_nl, "Model F.E" = e_jj_nl_fe) %>% gather(key = "Specification", value = "value", Baseline, "Model F.E") %>% group_by(firm, Specification) %>% summarise("mean_el" = abs(mean(value)), "se" = sd(value)/sqrt(n()))
  ...
  labs(title = "Nested logit absolute elasticities", 
       y = "(Absolute) Elasticities", x = "Firm ID", tag = "Graph X",
       caption = "Baseline corresponds to estimates from column 5, Table 2, Model.F.E. correpsonds to column 6. 
       Bars show two standard errors of the estimate") + 
    geom_errorbar(aes(ymin=mean_el-2*se, ymax=mean_el +2*se), width=.2,
                 position=position_dodge(.9))
```
```{r}
ggsave("nested_elast.png", height = 12, width = 20, units = "cm")
...
```{r}
# For fun =)
df %>% ggplot(aes(x = price, y = markup, color = as.factor(segment))) +
  geom_point(alpha = 0.2)

# subtracting a fixed fee such as the minimum markup (about 3), the relationship looks very different
df %>% ggplot(aes(x = price, y = (markup)/(price), color = as.factor(segment))) +
  geom_point(alpha = 0.2)
```
...
Table_5 <- full_join(pricing_table, pricing_table_new) %>% 
  mutate("Delta price (p.mille)" = 1000 * (avg_price_new/avg_price-1)) %>%
  select(firm, avg_price, avg_price_new, "Delta price (pct)", everything()) %>% 
  rename("Pre price" = avg_price, "Post price" = avg_price_new) %>% 
  column_to_rownames(var = "firm") %>% 
  xtable(digits = 2, caption = "Firm level price info")
```
