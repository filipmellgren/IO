...
fe_formula <- glue("log(price) ~", X_fe)
fe.fit <- lm(fe_formula, data = df)

stargazer(lm.fit, fe.fit, type = "text")
```
...
stargazer(lm.logit, fe.logit, type = "text")
```
...
df %>% select(firm, avg_price, avg_mc, avg_markup, avg_lerner) %>% 
