...
pricing_table_new <- merger_df %>% dplyr::select(firm, price_new, year) %>% filter(year == 1998) %>% group_by(firm)%>% mutate(AVG_price = mean(price_new, na.rm = TRUE)) %>% distinct(firm, .keep_all = TRUE) %>% ungroup() %>% select(-year)

pricing_table_eff <- merger_df_9 %>% dplyr::select(firm, price_new, year) %>% filter(year == 1998) %>% group_by(firm)%>% mutate(AVG_price = mean(price_new, na.rm = TRUE)) %>% distinct(firm, .keep_all = TRUE) %>% ungroup() %>% select(-year)
