...
descriptive_table <- df %>%
  select(country, year, domestic, qu, price, princ, horsepower, fuel, pop, ngdp) %>% 
...
rbind(tmp_table, tmp_table) %>% arrange(Variable) %>% 
  filter(Variable != "country") %>% select(Variable, All, "1", "2", "3", "4", "5")
```
...
```
```{r}
# Fixed effects regression - hedonic enough?
...
```{r}
# TODO: Add a market share and market size for each segment. Do it as percentages
# TODO: Check in papers (Goldberg, verboven, https://www.jstor.org/stable/pdf/3087434.pdf) what descriptives do they present?
# TODO: maybe take the five highest and five lowest per a and b. Most popular cars and least popular car
# TODO: present log of the variables
...
df %>% mutate(mshare = mshare *100) %>% select(mshare) %>% map(function(x) summary_stats(x)) %>% summary_table()
...
# TODO: Add fixed effects Check what within means, and tewoways. Means we'll get twice as many elasticities.
...
# TODO: fix output. What are we supposed to show? This and estimates form f.e.
...
pre_post_table %>%
  ...
  geom_point() +
  geom_point(aes(x = as.factor("Post merger"),y = price_new)) +
  geom_point(aes(x = as.factor("Post merger"), y = efficient_price)) +
  theme_economist() + scale_fill_economist() + scale_color_economist() +
  annotate("text", x = 0.85, y = pre_post_table$price, label = as.character(pre_post_table$name)) +
  labs(title = "Price hikes caused by merger", y = "Average price") +
