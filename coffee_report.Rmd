...
```
```{r}
cormatdf <- df %>% select(Coffee, Quantity, Bean, Tea, Wage, Income)
...
We estimate $\lambda \approx $ `r round(lambda,3)`, which means the market is composed of 

$\frac{1}{\lambda} \approx $ `r round(1/lambda,2)` equally sized firms.
