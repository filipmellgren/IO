...
```{r}
#summary(df$Coffee)
sapply(select(df,Quantity, Coffee, Tea, Wage, Bean), summary)
```
```{r}
cormatdf <- df %>% select(Coffee, Quantity, Bean, Tea, Wage)
...
df <- df %>% mutate("price roasted coffee" = Coffee/Coffee[1]*100,
              "price tea" = Tea/Tea[1]*100,
              "pc roasted coffee consumption" = Quantity/Quantity[1]*100,
...
```
```{r}
df %>% gather(`price roasted coffee`, `price coffee beans`, key = "index", 
              value = "price") %>% 
  ggplot(aes(x = time, y = price, color = index )) +
  geom_line() +
  theme_economist() + scale_colour_economist() +
  labs(y = "Indexed price", x = "Year", 
       title = "Bean prices hit roasted prices hard...",
       subtitle = "...but some of the impact is absorbed elsewhere" )
```
...
IV_spec <- ivreg(log(Quantity) ~ log(Coffee) + q1 + q2 + q3 + log(Tea) +
                       log(incom) | q1 + q2 + q3 + log(Tea) +
                       log(incom) + log(Bean) + log(Wage), data = df)
stargazer(no_controls, quarter_controls, tea_control, 
          income_control, IV_spec,  header = FALSE)
...
df %>% gather(L, L_adj, key = Type, value = Lerner_index) %>%
  ...
  geom_line() +
  theme_economist() + scale_colour_economist() +
  labs(title = "Mark ups are decreasing",
...
dummies <- lm(Coffee ~ c + 0 + q1 + q2 + q3 + q4 + 
                  log(Tea) + log(incom), data = df) # add controls
